...
import type { Project } from '@/types/project' // Import canonical Project type
...
export const getProjects = cache(async (): Promise<Project[]> => {
  ...
  return rawProjects.map((rawProject: RawJsonProject) => {
    ...
    const project: Project = {
      ...
      // Metrics, charts, details, testimonial, gallery might need more specific mapping
      // For now, let's assume they are compatible or handled if present in RawJsonProject
      // This part needs careful review based on actual JSON structure vs canonical Project type
      // The example below is a placeholder for metrics if it was structured differently in JSON
      // metrics: mapRawMetricsToCanonical(rawProject.project_metrics),
    };
