...
class InMemoryAnalyticsStorage implements AnalyticsStorage {
  ...
  async aggregate(timeRange: TimeRange, groupBy?: string): Promise<AnalyticsAggregation> {
    ...
    return {
      ...
      p75Value: values[Math.floor(values.length * 0.75)],
      p95Value: values[Math.floor(values.length * 0.95)],
      ratingDistribution: {
  ...
  private groupBy(data: EnhancedWebVitalsData[], key: string | ((item: any) => string)): Record<string, any> {
    const groups: Record<string, EnhancedWebVitalsData[]> = {}
    
    data.forEach(item => {
      const groupKey = typeof key === 'string' ? item[key as keyof EnhancedWebVitalsData] as string : key(item)
    ...
    Object.entries(groups).forEach(([groupKey, items]) => {
      ...
      result[groupKey] = {
        ...
        average,
        rating: getRating(items[0].name, average),
      }
    })
