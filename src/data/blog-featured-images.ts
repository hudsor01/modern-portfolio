/**
 * Canonical blog featured-image mapping. The 29 entries below are the
 * single source of truth for which Unsplash photo each post uses. They
 * are consumed by:
 *
 *   - drizzle/seed.ts                              — local dev seed
 *   - scripts/update-blog-featured-images.ts       — prod DB sync
 *
 * Adding/removing a post: edit the array here, then re-run
 * `bun run scripts/update-blog-featured-images.ts` against prod and
 * re-seed dev.
 *
 * Alt-text guideline: each `alt` is rendered both as the `<img alt>` for
 * screen readers AND as a visible `<figcaption>` overlay on the hero
 * (see `src/app/blog/_components/blog-post-layout.tsx`). Write alts that
 * are (a) accessible (describe what the image shows) AND (b) topically
 * coherent (work as a one-line caption for the post). Pure visual
 * descriptions like "professor at chalkboard" read as stock-photo
 * placeholders under topical post titles.
 */
...
export interface BlogFeaturedImage {
  ...
  readonly alt: string
  /**
   * `true` for posts staged in `drizzle/seed.ts` (DRAFT/SCHEDULED) that
   * aren't in the prod DB. `scripts/update-blog-featured-images.ts`
   * skips these so a clean prod run still exits 0. Remove the flag once
   * a post promotes to PUBLISHED in prod.
   */
...
export const BLOG_FEATURED_IMAGES: readonly BlogFeaturedImage[] = [
  ...
  // Seed-only (not currently PUBLISHED in prod): kept here for seed
  // consistency. Remove `seedOnly` when promoting to PUBLISHED.
  {
...
/**
 * Slug → `{ src, alt }` lookup. Built once at module load with explicit
