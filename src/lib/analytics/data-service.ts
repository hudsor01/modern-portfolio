...
class DataGenerator {
  ...
  generateTopPartnersData(count: number = 15): TopPartnerData[] {
    ...
    return partnerNames.slice(0, count).map((name, index): TopPartnerData => { // Explicit return type for map callback
      ...
      let selectedTier: { name: TopPartnerData['tier'], multiplier: number, probability: number } = tiers[tiers.length - 1]!;
      ...
      const partnerEntry: TopPartnerData = { // Explicitly type the object literal
...
class DataCacheService {
  ...
  getStats(): CacheStats { // Updated return type
    return {
...
interface CacheStats { // Defined CacheStats interface
  size: number;
  keys: string[];
}
...
export class AnalyticsDataService {
  ...
  async getAllAnalyticsData(): Promise<AllAnalyticsDataBundle> { // Updated return type
  ...
  getCacheStats(): CacheStats { // Updated return type
